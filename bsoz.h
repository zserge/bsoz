...
uint16_t r16(struct cpu *cpu, uint16_t addr) {
  return r8(cpu, addr) | ((uint16_t)r8(cpu, addr + 1) << 8);
}
uint16_t rb16(struct cpu *cpu, uint16_t addr) {
  return (r8(cpu, (addr & 0xff00) | ((addr + 1) & 0xff)) << 8) | r8(cpu, addr);
}

uint8_t push(struct cpu *cpu, uint8_t b) {
  return w8(cpu, 0x100 + cpu->s--, b);
}
uint8_t pop(struct cpu *cpu) { return r8(cpu, 0x100 + (++cpu->s)); }

void pushw(struct cpu *cpu, uint16_t w) {
  push(cpu, w >> 8);
  push(cpu, w & 0xff);
}
...
uint16_t abt(struct cpu *cpu) {
  return (uint16_t)zpg(cpu) | ((uint16_t)zpg(cpu) << 8);
}
uint16_t abx(struct cpu *cpu) { return abt(cpu) + cpu->x; }
uint16_t aby(struct cpu *cpu) { return abt(cpu) + cpu->y; }
uint16_t inx(struct cpu *cpu) {
  return rb16(cpu, (fetch(cpu) + cpu->x) & 0xff);
}
...
void shlm(struct cpu *c, uint16_t addr, uint8_t rot) {
  w8(c, addr, shl(c, r8(c, addr), rot));
}
void shrm(struct cpu *c, uint16_t addr, uint8_t rot) {
  w8(c, addr, shr(c, r8(c, addr), rot));
}
...
int step(struct cpu *c) {
  ...
  switch (op = disasm(c, fetch(c))) {
    ...
    /* Conditional jumps */
    case 0x90:
      c->pc += rel(c) * ((c->f & 0x01) == 0);
      break; /* BCC,REL,2,2/3,czidbvn */
    case 0xB0:
      c->pc += rel(c) * ((c->f & 0x01) != 0);
      break; /* BCS,REL,2,2/3,czidbvn */
    case 0xF0:
      c->pc += rel(c) * ((c->f & 0x02) != 0);
      break; /* BEQ,REL,2,2/3,czidbvn */
    case 0x30:
      c->pc += rel(c) * ((c->f & 0x80) != 0);
      break; /* BMI,REL,2,2/3,czidbvn */
    case 0xD0:
      c->pc += rel(c) * ((c->f & 0x02) == 0);
      break; /* BNE,REL,2,2/3,czidbvn */
    case 0x10:
      c->pc += rel(c) * ((c->f & 0x80) == 0);
      break; /* BPL,REL,2,2/3,czidbvn */
    case 0x50:
      c->pc += rel(c) * ((c->f & 0x40) == 0);
      break; /* BVC,REL,2,2/3,czidbvn */
    case 0x70:
      c->pc += rel(c) * ((c->f & 0x40) != 0);
      break; /* BVS,REL,2,2/3,czidbvn */

    /* Other jumps */
    case 0x4c: c->pc = abt(c); break; /* JMP,ABS,3,3,czidbvn */
    case 0x6c:
      c->pc = rb16(c, abt(c));
      break; /* JMP,IND,3,5,czidbvn */
    /*case 0x7c: break; [> JMP,AX,3,6,czidbvn <]*/
    case 0x20:
      pushw(c, c->pc + 1);
      c->pc = abt(c);
      break; /* JSR,ABS,3,6,czidbvn */
