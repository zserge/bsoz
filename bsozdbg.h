...
void dump(struct cpu *cpu) {
  // clang-format on
  printf(
      "A:%02x  X:%02x  Y:%02x  S:%02x[%02x %02x %02x %02x]  "
      "F:%02x[%c%c%c%c%c%c%c%c]  PC:%04x ",
      cpu->a, cpu->x, cpu->y, cpu->s, r8(cpu, 0x100 + cpu->s + 1),
      r8(cpu, 0x100 + cpu->s + 2), r8(cpu, 0x100 + cpu->s + 3),
      r8(cpu, 0x100 + cpu->s + 4), cpu->f, cpu->f & 0x80 ? 'N' : '.',
      cpu->f & 0x40 ? 'V' : '.', cpu->f & 0x20 ? '1' : '.',
      cpu->f & 0x10 ? 'B' : '.', cpu->f & 0x08 ? 'D' : '.',
      cpu->f & 0x84 ? 'I' : '.', cpu->f & 0x82 ? 'Z' : '.',
      cpu->f & 0x81 ? 'C' : '.', cpu->pc - 1);
  // clang-format on
}
